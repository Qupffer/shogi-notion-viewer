...
「無料で使える」と「ストレージおよび使用回数制限を気にしなくてよい」を優先しているため、GitHub Pagesを使用しています。  
つまり、`kif/` に置いた棋譜は、「見られてしまう」可能性があります。  
情報保護のための対策は、次章『使い方（Forkしないで使う方法・推奨）』に記載しています。  
...
さらに、 **普段使用しないアカウントでリポジトリ名を変えて使用する** と、意図しない形で内容が広く共有されるのを避けやすくなります。
...
`kif/` 配下に `.kif`をアップロード。  （.kifでファイルを作って編集欄にKIF形式をペーストでも可）  
フォルダの階層は自由。深くしてもOK。
